...
function Icon({ name, size = 20 }) {
  const paths = {
    ...
    paperclip: <path d="m20.5 11.5-8.7 8.7a5 5 0 0 1-7.1-7.1l9-9a3.5 3.5 0 1 1 5 5l-9 9a2 2 0 1 1-2.8-2.8l8.3-8.3"/>,
    smile: <><circle cx="12" cy="12" r="9"/><path d="M8 14s1.3 2 4 2 4-2 4-2M9 9h.01M15 9h.01"/></>,
    send: <path d="m21 3-7 18-3.8-7.2L3 10 21 3Zm-10.8 10.8L14 10"/>,
...
function Avatar({ person, compact = false }) {
  return <div className={`avatar ${person.tone} ${compact ? 'compact' : ''}`}>{person.initials}{person.online && <i />}</div>;
}
...
function App() {
  ...
  const send = async (event) => {
    ...
    setMessages((current) => ({
      ...current,
      [activeId]: [...(current[activeId] || []), message],
    }));
    setDraft('');
    ...
    try {
      ...
      // The optimistic local message remains visible when offline.
    }
  };

  const typingName = active?.name || 'người này';
      ...
      <main className="app">
        <aside className="rail">
          ...
          <div className="rail-actions">
            <button className="rail-button selected" aria-label="Tin nhắn">
              <Icon name="message" />
            </button>
            <button className="rail-button" aria-label="Thông báo">
              <Icon name="bell" />
            </button>
          </div>
          <button className="user-orb" aria-label="Tài khoản">AN</button>
        </aside>
        ...
        <aside className="inbox">
          <header className="inbox-header">
            ...
            <button className="round-button accent" aria-label="Cuộc trò chuyện mới">
              <Icon name="plus" />
            </button>
          </header>
          ...
          <nav className="contact-list">
            {visiblePeople.map((person) => (
              <button className={`contact ${person.id === activeId ? 'active' : ''}`} onClick={() => setActiveId(person.id)} key={person.id}>
          ...
          <div className="privacy-note">
            ...
            <span>
              <b>Cuộc trò chuyện riêng tư</b>
              <small>Qalo bảo vệ không gian của bạn.</small>
            </span>
          </div>
        </aside>
        ...
        <section className="chat">
          <header className="chat-header">
            <Avatar person={active} compact />
            <div className="chat-person">
              <h2>{active.name}</h2>
              <span>{active.online ? 'Đang hoạt động' : active.handle}</span>
            </div>
            <div className="header-tools">
              <button className="round-button" aria-label="Gọi video">
                <Icon name="video" />
              </button>
              <button className="round-button" aria-label="Thông tin">
                <Icon name="info" />
              </button>
              <button className="theme-switch" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} aria-label="Đổi giao diện">
          ...
          <div className="message-area">
            ...
            {(messages[activeId] || []).map((message) => (
              <div className={`message-row ${message.from === 'me' ? 'outgoing' : ''}`} key={message.id}>
                {message.from === 'them' && <Avatar person={active} compact />}
            ...
            <div className="typing">
              ...
              <b />
              <b />
              <span>{typingName} đang soạn tin</span>
            </div>
          </div>

          <form className="composer" onSubmit={send}>
            <button type="button" className="composer-action" aria-label="Đính kèm">
              <Icon name="paperclip" />
            </button>
            <input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={`Nhắn cho ${active.name}`} />
            <button type="button" className="composer-action" aria-label="Biểu tượng cảm xúc">
              <Icon name="smile" />
            </button>
            <button className="send-button" aria-label="Gửi tin nhắn" type="submit">
              <Icon name="send" size={19} />
            </button>
          </form>

          <p className="secure-footer">
            <Icon name="lock" size={13} /> Bảo vệ quyền riêng tư, từ cuộc trò chuyện đầu tiên.
          </p>
        </section>

        <aside className="profile">
          <button className="more-button" aria-label="Thêm tùy chọn">
            <Icon name="dots" />
          </button>
          <Avatar person={active} />
          <h3>{active.name}</h3>
          <p>{active.handle}</p>
          <button className="profile-button">Xem hồ sơ</button>
          <div className="profile-section">
            <h4>Cuộc trò chuyện</h4>
            <button>
              Ảnh, liên kết và tệp <span>12</span>
            </button>
            <button>
              Quyền riêng tư <Icon name="lock" size={15} />
            </button>
          </div>
